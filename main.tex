...
This service supports multiple query methods, including WHOIS (TCP 43), DNS (UDP 53), and HTTPS (TCP 443). The information obtained includes:
\begin{itemize}
    \item BGP Origin ASN
    \item BGP Peer ASN
    \item BGP Prefix
    \item Prefix Country Code (assigned)
    \item Prefix Registry (assigned)
    \item Prefix Allocation date
    \item ASN Country Code (assigned)
    \item ASN Registry (assigned)
    \item ASN Allocation date
...
In the context of this project, the Team Cymru service was essential for obtaining accurate WHOIS lookup results and mapping IP addresses to their respective ASNs. This allowed for a detailed analysis of the network routes and the identification of load balancers in the paths. The resolved IP addresses were then used to investigate the global distribution of load balancers and their impact on network performance and reliability.

By leveraging Team Cymru's comprehensive and frequently updated data, this research could accurately map the network topology and assess the role of load balancing in maintaining a stable and efficient Internet infrastructure.
